...
class Lexer:
    def __init__(self, source): 
        ...
        self.curr = 0
        self.line = 1
        self.tokens = [] # list of tokens to add to our list
        pass

    """
    when we call this we get the character at the current position from source and we advance
    current 
    ...
    def match(self, expected):
        ...
        if self.source[self.curr] != expected:
            return False
        self.curr = self.curr + 1 # if char is a match we consume
        return True
    ...
    def handle_identifier(self):
        while self.peek().isalnum() or self.peek == '_':
         ...
         # check if identifier matches a key in the keywords dictionary
    ...
    def tokenize(self):
        # Add tokens to the list as we scan them from the source
        while self.curr < len(self.source):
            ...
            if ch == '\n': self.line = self.line + 1
                ...
                raise SyntaxError(f'Error at {ch}: Unexpected Character.')            
